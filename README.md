...
| [**Bill Categoriser**](https://github.com/NickkkLian/bill-categoriser) | Turns a year of messy small-business bills into a five-sheet reconciliation workbook — every row traceable to its source, totals reconciled to the cent, and a check command that proves the invariants hold and turns red when they don't | Python stdlib, seeded synthetic data, GitHub Actions on Windows/macOS/Linux |
| [**Missed-Call Booking**](https://github.com/NickkkLian/missed-call-booking) | An importable n8n workflow that follows up a missed call by SMS and WhatsApp, collects the booking request, and writes to the calendar only after a staff approval every path must pass through — with an offline simulator covering 22 scenarios | n8n JSON, Node stdlib simulator, no external services |
